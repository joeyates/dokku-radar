...
- [x] 2. Add tests in `test/dokku_radar/cli/diagnose_test.exs`: extend `stub_commands_enter_app_run` to handle the `/metrics` URL; add a passing test (all metrics present with data) and a failing test (one or more metrics absent or empty).
